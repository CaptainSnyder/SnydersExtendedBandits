...
function BanditClanMain:onAvatarListChange()
    ...
    -- CUSTOM DIALOGUE PATCH
    self.customDialogueBtn = ISButton:new(leftX - UI_BORDER_SPACING, topY + rowY, 130, BUTTON_HGT, "Custom Dialogue", self, BanditClanMain.onCustomDialogueClick)
    self.customDialogueBtn:initialise()
    self.customDialogueBtn:instantiate()
    self:addChild(self.customDialogueBtn)
    rowY = rowY + BUTTON_HGT + 8
...
function BanditClanMain:onCustomDialogueClick(button)
    self:saveConfig()
    self:cleanUp()

    local modalWidth, modalHeight = 820, 500
    local screenWidth, screenHeight = getCore():getScreenWidth(), getCore():getScreenHeight()
    local modalX = (screenWidth / 2) - (modalWidth / 2)
    local modalY = (screenHeight / 2) - (modalHeight / 2)
    local cid = self.cid

    local modal = BanditDialogueMain:new(modalX, modalY, modalWidth, modalHeight, cid, function()
        local sw2, sh2 = getCore():getScreenWidth(), getCore():getScreenHeight()
        local m2 = sw2 > 1900 and 100 or 0
        local w2, h2 = sw2 - m2, sh2 - m2
        local x2, y2 = (sw2 / 2) - (w2 / 2), (sh2 / 2) - (h2 / 2)
        local back = BanditClanMain:new(x2, y2, w2, h2, cid)
        back:initialise()
        back:addToUIManager()
    end)
    modal:initialise()
