...
-- RANDOM DIALOGUE PATCH: ambient chatter unrelated to any specific game event.
-- Only ever plays custom clan dialogue lines (there is no generic "RANDOM"
-- category), so clans with nothing configured for this trigger stay silent.
local function ManageRandomChatter(bandit, brain)
    if not brain.randomChatterCooldown then
        brain.randomChatterCooldown = 60 + ZombRand(120)
    end

    brain.randomChatterCooldown = brain.randomChatterCooldown - 0.01

    if brain.randomChatterCooldown <= 0 then
        brain.randomChatterCooldown = 60 + ZombRand(120)
        Bandit.Say(bandit, "RANDOM")
    end
end
...
local function ManageCombat(bandit)
    ...
    -- CLAN BATTLE POC (experimental) - AreEnemies() already makes two different
    -- hostile clans mutually hostile, but target selection below is pure
    -- closest-wins, so a nearby player still out-prioritizes a rival bandit.
    -- Shrinking the effective distance to a player-target here just tips that
    -- comparison toward the clan fight instead of pulling bandits off it and
    -- onto the player. Tune the 0.4 multiplier or delete this block entirely.
    if enemyCharacter and instanceof(enemyCharacter, "IsoPlayer") then
        bestDist = bestDist * 0.4
    end
...
local function OnBanditUpdate(zombie)
    ...
    -- MANAGE RANDOM AMBIENT CHATTER
    ManageRandomChatter(bandit, brain)
