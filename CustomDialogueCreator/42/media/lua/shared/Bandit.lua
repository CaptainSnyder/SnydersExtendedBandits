                                ...
                                if def.attachments then
                                    for k, v in pairs(def.attachments) do
                                        if k == attachmentType then
                                            ...
                                            weapon:setAttachedToModel(v)
                                            weapon:setAttachedSlotType(def.type)
                                            -- print ("Attaching " .. brain.weapons[slot].name .. " to " .. def.type .. " slot " .. v)
                                            break
                                        end
                                    end
                                end
...
function Bandit.Say(zombie, phrase, force)
    ...
    if dist <= 14 then
        ...
        -- CUSTOM DIALOGUE PATCH: check for clan-specific custom lines for this trigger
        -- before falling back to the mod's generic canned lines/voice clips.
        if brain.cid and BanditDialogueCustom then
            local customLines = BanditDialogueCustom.GetFromClanAndTrigger(brain.cid, phrase)
            if #customLines > 0 then
                -- shuffle so we don't always check them in the same order
                for i = #customLines, 2, -1 do
                    local j = ZombRand(i) + 1
                    customLines[i], customLines[j] = customLines[j], customLines[i]
                end

                for _, line in ipairs(customLines) do
                    local chance = tonumber(line.chance) or 0
                    if ZombRand(100) < chance and line.text and line.text ~= "" then
                        if SandboxVars.Bandits.General_Captions then
                            if brain.hostile or brain.hostileP then
                                zombie:addLineChatElement(line.text, 0.8, 0.1, 0.1)
                            else
                                zombie:addLineChatElement(line.text, 0.1, 0.8, 0.1)
                            end
                        end
                        brain.speech = 4
                        return
...
function Bandit.GetHairStyle(female, idx)
    if female then
        return Bandit.femaleHairStyles[idx]
    else
        return Bandit.maleHairStyles[idx]
    end
    --[[
