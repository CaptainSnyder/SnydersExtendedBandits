...
local function banditize(zombie, bandit, clan, args)
    ...
    brain.fullname = args.fullname or BanditNames.GenerateName(bandit.general.female)
...
local function spawnRestore(brain)
    ...
    args.stage = brain.program.stage
        ...
        if spawnConfig and spawnConfig.dayStart and spawnConfig.dayEnd then
            if day >= spawnConfig.dayStart and day <= spawnConfig.dayEnd then
                ...
                if spawnRandom < spawnChance then
                    ...
                    local args = {}
                    args.cid = cid
                    -- FORCE-SPAWN PATCH: use the lenient placement check (generateSpawnPointHere,
                    -- only requires the tile to be loaded) instead of the strict version
                    -- (generateSpawnPointUniform, also checks occupied/safehouse/player-distance),
                    -- which was failing consistently. Distance restored to the original 55-65 tile ring.
                    local theta = ZombRandFloat(0, 2 * math.pi)
